    ...
    <div class="specDetail" v-if="specification.length">
        ...
        >
            ...
            <el-table-column align="center" label="价格">
                <template prop="goods_price" slot-scope="scope">
                    <el-input-number
                        size="mini"
        ...
        <div class="specDetail_parent_footer">
            <div>批量设置：</div>
            <div
                v-show="!batchSetPriceShow"
                @click="batchSetPriceShow = true"
                style="
            ...
            <div v-show="batchSetPriceShow" style="display: flex">
                <el-input-number
                    size="mini"
                    :min="0.01"
                    v-model="batchSetPrice"
                ></el-input-number>
                <div
                    @click="handleSaveBatchSetPrice"
                    style="
                        text-decoration: underline;
                        color: #eb7044;
                        margin-left: 5px;
            ...
            if (this.isEditSpecArr.length) {
                ...
                for (let index = 0; index < this.newList.length; index++) {
                    this.tableDataCopy.push({
                        goods_spec_value_key:
                            this.newData.length > 2
                                ? this.newList[index][0].goods_spec_value_key +
                                  "-" +
                                  this.newList[index][1].goods_spec_value_key +
                                  "-" +
                                  this.newList[index][2].goods_spec_value_key
                                : this.newData.length > 1
                                ? this.newList[index][0].goods_spec_value_key +
                                  "-" +
                                  this.newList[index][1].goods_spec_value_key
                ...
                if (this.tableData.length) {
                    ...
                    for (let i = 0; i < this.tableDataCopy.length; i++) {
                        ...
                        if (nowIndex !== -1) {
                            this.tableDataCopy[i].goods_price = this.tableData[
                                nowIndex
                            ].goods_price;
                            this.tableDataCopy[i].bar_code = this.tableData[
                                nowIndex
                            ].bar_code;
                            this.tableDataCopy[i].goods_stock = this.tableData[
                                nowIndex
                            ].goods_stock;
                        }
